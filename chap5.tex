...
\item For systematic uncertainty check, add a free parameter ($a$), which is commonly multiplied by the widths of the signal Gaussians, serving as a scale factor of the resolution that parametrizes the difference between data and MC signal shapes. However, in the nominal fit, this is set to be unity ($a = 1$), which means the widths of the data signal are set to be identical to the ones of the MC signal.
...
We can see that most of the candidates are either background-like (near 0) or signal-like (near 1). But some candidates are in between. Hence, the \textit{Splot} weighted efficiency correction factor $\langle\frac{1}{\alpha \times \epsilon} \rangle'$ is given by
...
Figure \ref{BmesonDataCent} shows $B^0_s$ and $B^+$ $p_T$ integrated cross section as a function of average number of participants $\langle N_{part}\rangle$, with the corresponding PbPb collision centralities labeled in the plot
