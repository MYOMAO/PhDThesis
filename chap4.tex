...
Here, we should note that since we do not have hadronic PID for the kaons, we assume the track to be kaons and assume the charged kaon PDG mass to the tracks \cite{AlphaTheoEx} to the tracks. Also, the invariant mass of muon pair is constrained to the nominal $J/\psi$ meson PDG mass ($m_{J/\psi}$ = 3.096916 GeV/c$^2$) \cite{AlphaTheoEx} instead of a distribution of the dimuon mass $m_{\mu\mu}$. 
...
Figure \ref{DecayTopo} shows the definition of topological variables of $B^0_s$ and $B^+$ decay chains

\begin{figure}[h]
\begin{center}
\includegraphics[width= 0.48\textwidth]{Figures/Chapter4/BsDecay.png}
\includegraphics[width= 0.48\textwidth]{Figures/Chapter4/BPDecay.png}
\caption{The definition of topological variables in the decay of $B^0_s \rightarrow J/\psi \phi \rightarrow \mu^+\mu^- K^+K^-$ (left) and $B^+ \rightarrow J/\psi K^+ \rightarrow \mu^+\mu^- K^+ $ (right) are schematically shown above.}
\label{DecayTopo}
\end{center}
\end{figure}
...
In this analysis, our goal is to separate B meson signal out of the background. Therefore, it is a classification problem. We can apply supervised machine learning to train the computer and let them find the optimal selection for B-meson analyses.   

\subsection{Boosted Decision Tree Algorithm}

Nowadays, machining learning has become sophisticated. There are many well developed machining learning algorithms. In order to test their performance, we following 
...
To check the performance of the event, track, and muon selections, we look at the dimuon and dikaon invariant mass distributions to see if $J/\psi$ and $\phi$ resonance are observed. Figure \ref{mesonpeakPbPb} shows the after apply the selections
