...
Another source of uncertainties on the selection efficiency is the statistics of the MC samples. Ideally, this uncertainties should be 0 because we can in principle simulate as many MC events as we want. However, in reality, we only generated about 2.5 million MC events, which is finite. Particularly, when we bin the 2D efficiency vs B-meson $p_T$ and $|y|$ map, due to the limited acceptance and fine binning, very low $p_T$ B meson will have very few candidates.
