...
Figure \ref{} shows the definition of signal and background regions in $B^0_s$ and $B^+$ invariant mass plot. 
...
For $B^0_s$  Our optimal selection returns us with an S = 47 and B = 2, which has a remarkable background-to-signal rejection of $10^3 : 1$.



Table \ref{BsSig} and \ref{BPSig} documents the calculated $S$, $B$, and $Sig$ with optimal BDT selections in each $p_T$ bin for $B^0_s$ and $B^+$ respectfully.
